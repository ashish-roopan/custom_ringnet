...
class FAN(object):
    ...
    def get_3d_landmarks(self, img):
        lmk3d, mesh, pose = self.model.get_all_outputs(img)
        xs = lmk3d[0][0,:]
        ys = lmk3d[0][1,:]
        zs = lmk3d[0][2,:]
        lmk3d = np.array([[x,y] for x,y,z in zip(xs,ys,zs)])
        return lmk3d
    ...
    def run(self, image):
        ...
        '''
        out = self.get_3d_landmarks(image)
        if out is None:
            return [0], 'kpt68'
        else:
            kpt = out
